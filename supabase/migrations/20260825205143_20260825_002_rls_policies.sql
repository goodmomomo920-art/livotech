/*
# LivoTech RLS Policies

Enables RLS on all tables and creates all access policies.

## Security Changes
- Public catalog tables (products, categories, product_types, addons, etc.): anon+authenticated read, admin write
- Customer tables (orders, subscriptions, etc.): owner-scoped CRUD via auth.uid()
- Admin-only tables (coupons, admin_activity_logs): is_admin() check
- Product files: only visible to owners of that product or admins
- Settings: public read, admin write
- Profiles: owner read/update, admin read

## Important Notes
1. Public catalog is readable by anon (no-auth browsing works)
2. Customer data is strictly owner-scoped — no cross-customer access
3. Admin access via is_admin() SECURITY DEFINER function checking profiles.role
4. Product files require active customer_products ownership to read
*/
